...
if sorted(JSON_FIELDS.keys()) != sorted(OVS_MATCH_FIELDS.keys()):
   print("ERROR: Key mismatch between JSON_FIELDS and OVS_MATCH_FIELDS:\n ".set(list(JSON_FIELDS.keys())).symmetric_difference(set(OVS_MATCH_FIELDS.keys())))
   exit(2)
...
if len(list(TABLE_SIZE.keys())) > MAX_SUPPORTED_TABLES:
   error("HPE Aruba switches support a maximum of "+str(MAX_SUPPORTED_TABLES)+" tables, but "+str(len(list(TABLE_SIZE.keys())))+" were used:\n "+str(list(TABLE_SIZE.keys())))
